...
export class SpaceService {
  ...
  public async updateSpace(
    ...
    try {
      const space = await Space.findByIdAndUpdate(spaceId, updatedSpace, {
        new: true,
      });
      return space;
    } catch (error) {
  ...
  public async createSpaceLog(
    spaceId: ISpace | Types.ObjectId,
    message: string,
    type?: SpaceLogType
  ): Promise<ISpaceLog> {
    try {
  ...
  public async setUnderMaintenance(
    spaceId: string,
    expectedMaintenanceEnd?: Date,
    log?: ISpaceLog | string
  ): Promise<ISpace | null> {
    try {
      const space = await Space.findById(spaceId);
      if (!space) {
      ...
      if (log) {
        if (typeof log === "object") {
          if (log.id !== undefined) {
            ...
            maintenanceLog = await this.createSpaceLog(
              space,
              log.message,
              log.type || SpaceLogType.Maintenance
            );
          }
